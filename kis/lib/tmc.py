...
def	NewTmc(user_kod,tema,text,group):
    user_kod = user_kod.encode("utf-8")
    tema = tema.encode("utf-8")
    text = text.encode("utf-8")
    group = group.encode("utf-8")
    cursor = connections['default'].cursor()
    cursor.execute("SELECT t_NewTmc('%s','%s','%s','%s');" % (user_kod,tema,text,group))
    transaction.commit_unless_managed()
    data = cursor.fetchone()
...
### --- Изменение текста заявки ТМЦ ---
def	EditTmc(tmc_kod,tema,text):
    tmc_kod = tmc_kod.encode("utf-8")
    tema = tema.encode("utf-8")
    text = text.encode("utf-8")
    cursor = connections['default'].cursor()
    cursor.execute("UPDATE t_tmc SET t_tema=btrim('%s'), t_note_text=btrim('%s') WHERE t_rec_id=%s AND t_status_kod=0;" % (tema,text,tmc_kod))
    transaction.commit_unless_managed()
...
#### --- Удаление из группы eamil рассылки ---
